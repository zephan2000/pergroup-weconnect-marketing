    ...
    <div className="space-y-4 pb-4 font-sora">
      <div className="glass-card rounded-xl p-5 relative overflow-hidden border-l-4 border-l-amber">
        ...
        <div className="relative z-10 flex items-center gap-4">
          <div className="w-14 h-14 rounded-full bg-amber flex items-center justify-center font-extrabold text-pg-text text-lg">
            PG
          </div>
          <div>
            <h3 className="font-bold text-pg-text text-lg">{weconnect.enterpriseUser}</h3>
      ...
      <div>
        <h4 className="font-bold text-sm text-pg-text mb-2">{weconnect.myPosts}</h4>
        {myPosts.map((r) => (
          <div key={r.title} className="glass-card rounded-xl p-3 mb-2 flex items-center justify-between">
            <div>
              <div className="text-sm font-semibold text-pg-text">{r.title}</div>
            </div>
      ...
      <div>
        <h4 className="font-bold text-sm text-pg-text mb-2">{weconnect.companyProfile}</h4>
        <div className="glass-card rounded-xl p-4 space-y-3">
          {companyFields.map((item) => (
            <div key={item.label} className="flex items-center justify-between">
              <span className="text-xs text-muted">{item.label}</span>
              <span className="bg-amber/10 text-deep-orange text-xs font-semibold px-2 py-0.5 rounded-md">
                {item.value}
              </span>
            </div>
          ))}
      ...
      <div>
        <h4 className="font-bold text-sm text-pg-text mb-2">{weconnect.settingsHeading}</h4>
        <div className="glass-card rounded-xl divide-y divide-line">
          {[
            ...
            <div key={s.label} className="p-3 flex items-center justify-between">
              <span className="text-sm text-pg-text">{s.label}</span>
              {s.comingSoon ? (

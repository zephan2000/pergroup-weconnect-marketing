...
/**
 * NeedsScreen — "Post a Need" + "Share an Offering" cards with preview content.
 *
 * Tab labels, CTA copy, recent-needs heading, preview/coming-soon badges all
 * come from the WeConnectSettings global (passed in from layout). Sample
 * preview content (recent-needs cards, urgent alert preview) stays hardcoded
 * — it's mock content, not editorial copy.
 *
 * Spaces browser is preserved in src/components/weconnect/SpacesContent.tsx
...
const recentNeedsByLocale = {
  en: [
    { icon: Package, title: 'Alternative logistics supplier — Southeast Asia', tags: ['Logistics', 'Procurement'], time: '2d ago' },
    { icon: TrendingUp, title: 'FX hedging advisory for SGD/CNY exposure', tags: ['Finance', 'Risk'], time: '4d ago' },
  ],
  zh: [
    { icon: Package, title: '寻找东南亚替代物流供应商', tags: ['物流', '采购'], time: '2天前' },
    { icon: TrendingUp, title: '人民币对新元汇率对冲咨询', tags: ['金融', '风险'], time: '4天前' },
  ],
}
  ...
  return (
    <div className="space-y-4 pb-4">
      ...
      <div className="relative">
        <div className="absolute inset-x-0 top-0 z-10 flex justify-center">
          <span className="bg-bg-2 text-muted text-[10px] font-semibold px-3 py-1 rounded-full -translate-y-1/2">
            {weconnect.comingSoonLabel}
          </span>
        </div>
        <div className="opacity-60 pointer-events-none select-none space-y-3 mt-2">
          <p className="text-xs text-muted mb-2">{weconnect.recentNeeds}</p>
          {recentNeeds.map((r) => (

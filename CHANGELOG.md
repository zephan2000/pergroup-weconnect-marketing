...
[2026-03-18] FIX [middleware.ts] — Logout cookie deletion: switched from NextResponse.redirect() to NextResponse.next() with Set-Cookie header that expires the payload-token cookie. Redirect responses don't reliably deliver Set-Cookie during client-side navigation. Pass-through with explicit cookie expiry (matching httpOnly, secure, sameSite: lax, path: /) ensures the browser clears the JWT cookie.

...
[2026-05-07] DATA [scripts/backfill-globals-published-status.sql + cms.* prod] — Bug fix: after enabling `versions: { drafts: true }` on globals, the Drizzle schema push filled the new `_status` enum column on existing rows with the enum's first value (`'draft'`). With the main record stuck at draft, Payload had no published baseline to gate against, so editor "Save Draft" actions leaked straight onto the public site (the symptom user reported). Fix is a one-shot UPDATE that flips `_status` from `'draft'` → `'published'` on the 6 global rows that have data (nav, footer, weconnect, requirement_form, offering_form, contact_form). Idempotent — only touches rows still at `'draft'`. `platform_settings` and `site_settings` have no row yet; they'll be created with the right status when first edited. Applied to prod 2026-05-07. Verified `cms.<global>._status = 'published'` post-run; this matches the existing `cms.pages._status` shape that has been working with drafts since Phase 5b.2.

[2026-05-07] ADD [src/components/DraftModeBanner.tsx, src/app/(marketing)/layout.tsx] — Persistent top banner shown on the full marketing site whenever `draftMode().isEnabled` is true. Editors browsing prod in the same browser as their `/admin` session see drafts (because the `__prerender_bypass` / `__next_preview_data` cookies set by `/api/draft` apply to every tab) and previously had no visual indication they weren't seeing the published site. The banner is amber, sticky `top-0 z-[1000]`, includes an "Exit draft" link to `/api/exit-draft`, and is hidden when rendered inside the Payload Live Preview iframe (detection: `window.self !== window.top` after mount; cross-origin access throws → also treated as framed). Plain `<a>` for the exit link on purpose so the cookie-clearing redirect goes through a full server round-trip.

[2026-05-07] STYLE [src/components/blocks/HeroBlock.tsx] — Tightened the Hero subtitle (the brand pillar line) from `tracking-widest` (0.1em) to `tracking-normal` (0). The bullet separators (·) already provide enough visual breathing room between "Tech Innovation · Business Empowerment · Human Care"; the prior letter-spacing was spreading it across the full hero column. No data change.

[2026-05-07] DATA [scripts/backfill-en-array-locales.sql + cms.* prod] — Bug fix: editor was unable to publish any draft on the home page because Payload validation flagged 14 invalid fields — the EN locale rows were missing entirely from three required+localized array fields (4× About milestones `title`, 4× Clients partner_types `title`, 6× Clients regions `name`). Phase 5b.2 had only inserted ZH rows when migrating those previously-hardcoded arrays into CMS. The script INSERTs the matching EN row for every array item that has a ZH row but no EN row, mapping ZH title/name → original English string recovered from git (commit e318a71~1, where the arrays were last hardcoded). Same fix is applied to the `_pages_v_blocks_*` version snapshots so historical drafts also validate. Idempotent via `WHERE NOT EXISTS`. Applied to prod 2026-05-07: 14 main rows + 182 version rows backfilled. Editor can now Publish without errors. Hero → Subtitle (EN) is the only remaining missing EN field on the home page; that one is editor-fillable in admin.
